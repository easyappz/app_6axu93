...
function Home({ onRequireAuth }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoadingTop(true);
        const data = await getTopListings({ sort: 'views', limit: 12 });
  ...
  return (
    <section data-easytag="id1-react/src/pages/Home.jsx" className="space-y-12">
      <div data-easytag="id2-react/src/pages/Home.jsx" className="text-center space-y-4">
        <h1 data-easytag="id3-react/src/pages/Home.jsx" className="text-3xl md:text-5xl font-semibold tracking-tight">Анализируйте объявления с Avito</h1>
        <p data-easytag="id4-react/src/pages/Home.jsx" className="text-neutral-600 max-w-2xl mx-auto">Вставьте ссылку на объявление Avito и получите страницу с обсуждениями и счётчиком просмотров.</p>
      </div>
      ...
      <form data-easytag="id5-react/src/pages/Home.jsx" onSubmit={submitUrl} className="max-w-2xl mx-auto flex items-center gap-3">
        ...
        />
        <button data-easytag="id7-react/src/pages/Home.jsx" type="submit" disabled={processing} className="px-6 py-4 rounded-2xl bg-black text-white hover:bg-neutral-800 transition-colors disabled:opacity-60">{processing ? 'Обработка…' : 'Проанализировать'}</button>
      </form>
      ...
      <div data-easytag="id9-react/src/pages/Home.jsx" className="space-y-6">
        ...
        <div data-easytag="id11-react/src/pages/Home.jsx" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {!loadingTop && listings.map((item) => (
            <a data-easytag="id14-react/src/pages/Home.jsx" key={item.id} href={`/ad/${item.id}`} className="group block rounded-2xl overflow-hidden bg-white border hover:shadow-soft transition-shadow">
